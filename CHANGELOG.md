...
- Hardened remote conformance ASGI response validation for malformed response
...
- Hardened strict JSON decoding against duplicate object keys.
...
- Hardened store verification against malformed persisted index snapshot item
  counts.
- Hardened store manifest commitment-state validation against unsupported
  backends and incoherent enabled or disabled metadata.
